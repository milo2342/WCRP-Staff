...
async function sendStaffPromotionNotification({ guild, target, oldRank, newRank, by, reason, action='promotion' }) {
  ...
  const title = isAdded
    ? 'Welcome to the WCRP Staff Team'
    : `Staff Promotion — ${newRank}`;
  const description = isAdded
    ? `You have officially joined the **WCRP Staff Team** in **${guild.name}**.`
    : `Your staff rank in **${guild.name}** has been successfully updated.`;
  ...
  const buildBaseEmbed = () => new EmbedBuilder()
    ...
    .addFields(
      ...
      { name: 'Status', value: isAdded ? 'Staff access granted' : 'Promotion completed', inline: true },
...
function buildTmodEligibilityEmbed(guild, target, userRecord, member=null, statusText=null) {
  ...
  return new EmbedBuilder()
    ...
    .setTitle(statusText?.includes('PROMOTED') ? '✅ Trial Moderator Promoted!' : '🎯 Trial Moderator Promotion')
    .setDescription(eligible || statusText?.includes('PROMOTED')
      ? `A Trial Moderator promotion interview is being processed for **${target.username}**.`
      : `**${target.username}** does not currently meet every Trial Moderator promotion requirement.`)
    .setThumbnail(target.displayAvatarURL({size:256}))
    .setColor(statusText?.includes('PROMOTED') ? 0x22c55e : (eligible ? 0xf59e0b : 0x64748b))
    .addFields(
      ...
      { name:'📌 Promotion Requirement', value:`**2+ hours**, **no active strikes**, and an **Administrator or higher must pass the interview**. Moderator is NOT assigned until the interview is passed.`, inline:true }
    )
...
async function handleTmodCompleteButton(interaction) {
  ...
  const updated=buildTmodEligibilityEmbed(guild,target,record,targetMember,'✅ **INTERVIEW PASSED • PROMOTED TO MODERATOR**')
    .setDescription(`**${target.username}** has passed the Trial Moderator interview and has now been promoted to **${MODERATOR_RANK}**.`)
    .addFields(
      { name:'👨‍💼 Interview Passed By', value:`<@${interaction.user.id}>`, inline:true },
  ...
  const row=new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId(`tmod_passed:${userId}`).setLabel('✅ Interview Passed • Moderator Assigned').setStyle(ButtonStyle.Success).setDisabled(true)
  );
...
**Assigned by:** ${interaction.user}`)]});
  ...
  if (name==='tmodpromos') {
    ...
    if(!outChannel || outChannel.type!==ChannelType.GuildText) {
      return interaction.editReply({embeds:[err('TMod Promotions Channel Not Set','Use `/tmodpromotions channel` to choose the channel where Trial Moderator interview requests will be posted.') ]});
    }
    ...
    if(requested){
      ...
      if(!r.rank) { const inferred=inferRankFromMember(m); if(inferred){r.rank=inferred;saveStore();} }
      ...
      for(const m of interaction.guild.members.cache.values()){
        ...
        if(!r.rank && inferred){r.rank=inferred;}
    ...
    for(const [id,r,m] of candidates.slice(0,25)){
      ...
      try{
        await target.send({embeds:[new EmbedBuilder().setAuthor({name:'PSRP Staff Utilities',iconURL:interaction.client.user.displayAvatarURL()}).setTitle('🎯 Trial Moderator Promotion Interview').setDescription(`Congratulations — you have met the **${TMOD_PROMOTION_HOURS} hour** and **no active strike** requirements for a Trial Moderator promotion interview.\n\nAn **Administrator or higher** will need to interview you. Your **Moderator role will NOT be assigned until the interview is passed**.`).addFields({name:'Hours',value:`**${Number(r.hours||0).toFixed(2)}h / ${TMOD_PROMOTION_HOURS}h**`,inline:true},{name:'Status',value:'⏳ Interview pending',inline:true},{name:'Interview Channel',value:`${outChannel}`,inline:false}).setColor(0xf59e0b).setFooter({text:`PSRP Staff Utilities • ${interaction.guild.name}`}).setTimestamp()]});
      }catch{}
      posted++;
    }
    return interaction.editReply({embeds:[ok('TMod Interviews Engaged',`Posted **${posted}** Trial Moderator interview${posted===1?'':'s'} in ${outChannel}.\n\n**No Moderator roles were assigned.** An Administrator or higher must click **Interview Passed** after the interview.`)]});
  }
  if (name==='promote-all-tmods') {
    ...
    saveStore();
    if(!candidates.length) return interaction.editReply({embeds:[embed('No Trial Moderators Ready','There are no Trial Moderators with at least **2 hours**, **no active strike**, and the required TMod rank.',0x64748b)]});
    let posted=0;
    for(const [id,r,m] of candidates.slice(0,25)){
      const target=m.user;
      const card=buildTmodEligibilityEmbed(interaction.guild,target,r,m);
      const row=new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`tmod_complete:${id}`).setLabel('📝 Interview Passed').setStyle(ButtonStyle.Primary));
      const msg=await outChannel.send({embeds:[card],components:[row]});
      try{const thread=await msg.startThread({name:`${target.username} - Trial Mod Interview`,autoArchiveDuration:1440,reason:'Trial Moderator promotion interview'});await thread.send({content:`${target} — **Interview required before promotion.** An Administrator or higher must conduct the interview and click **Interview Passed** after completion. **No Moderator role has been assigned yet.**`});}catch{}
      try{await target.send({embeds:[new EmbedBuilder().setAuthor({name:'PSRP Staff Utilities',iconURL:interaction.client.user.displayAvatarURL()}).setTitle('🎯 Trial Moderator Promotion Interview').setDescription(`You are eligible for a Trial Moderator promotion interview. An **Administrator or higher** must interview you first. **Moderator will only be assigned after the interview is passed.**`).addFields({name:'Hours',value:`**${Number(r.hours||0).toFixed(2)}h / ${TMOD_PROMOTION_HOURS}h**`,inline:true},{name:'Status',value:'⏳ Interview pending',inline:true},{name:'Interview Channel',value:`${outChannel}`,inline:false}).setColor(0xf59e0b).setTimestamp()]});}catch{}
      posted++;
    }
    return interaction.editReply({embeds:[ok('TMod Interviews Engaged',`Started **${posted}** interview workflows in ${outChannel}. **No Moderator roles were assigned automatically.**`)]});
  }
