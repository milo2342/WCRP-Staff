...
async function sendStaffPromotionNotification({ guild, target, oldRank, newRank, by, reason, action='promotion' }) {
  ...
  const title = action === 'added' ? 'Staff Member Added' : 'Staff Rank Updated';
  const description = action === 'added'
    ? `${target} has been added to the staff team.`
    : `${target} has received a staff rank update.`;
  const color = action === 'added' ? 0x22c55e : 0x38bdf8;
  if (channelId) {
    ...
    if (channel?.isTextBased()) {
      const e = new EmbedBuilder()
        ...
        .addFields(
          ...
          { name: 'Staff Discord', value: STAFF_DISCORD_INVITE, inline: false }
        )
  ...
  const dm = new EmbedBuilder()
    ...
    .setTitle(action === 'added' ? 'Welcome to the Staff Team' : 'Staff Rank Updated')
    .setDescription(action === 'added'
      ? `You have been added to the staff team in **${guild.name}**.`
      : `Your staff rank in **${guild.name}** has been updated.`)
    .setColor(color)
    .setThumbnail(target.displayAvatarURL({ size: 256 }))
    .addFields(
      { name: 'Previous Rank', value: oldRank || 'Unranked', inline: true },
    ...
    .setFooter({ text: 'WCRP Staff Team' })
    .setTimestamp();
...
async function sendStaffRemovalNotification({ guild, target, oldRank, by, reason, source='staff removal' }) {
  ...
  const dm = new EmbedBuilder()
    .setAuthor({ name: 'PSRP Staff Utilities', iconURL: guild.client.user.displayAvatarURL() })
    .setTitle('Staff Status Updated')
    .setDescription(`You have been removed from the staff team in **${guild.name}**.`)
    .setColor(0xef4444)
    .setThumbnail(target.displayAvatarURL({ size: 256 }))
    .addFields(
      { name: 'Previous Rank', value: oldRank || 'Unranked', inline: true },
      { name: 'Removed By', value: `<@${by}>`, inline: true },
      { name: 'Reason', value: reason || 'No reason provided.', inline: false },
      { name: 'What Happens Next', value: 'Your staff rank and staff-team access have been removed. If this was unexpected, contact your management team.', inline: false }
    )
    .setFooter({ text: 'WCRP Staff Team' })
    .setTimestamp();
  const row = new ActionRowBuilder().addComponents(
...
client.on('interactionCreate', async i=>{if(i.isAutocomplete()){try{await handleAutocomplete(i);}catch(e){console.error(e);try{await i.respond([]);}catch{}}return;}if(!i.isChatInputCommand())return;try{await handle(i);}catch(error){console.error(error);await reply(i,{embeds:[err('Command Error','Something went wrong while running this command.')],ephemeral:true});}});
