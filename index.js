...
add(new SlashCommandBuilder().setName('promote-all-tmods').setDescription('Promote every eligible Trial Moderator with 2+ hours and no active strike'));
...
async function sendStaffPromotionNotification({ guild, target, oldRank, newRank, by, reason, action='promotion' }) {
  ...
  const color = isAdded ? 0x22c55e : 0x8b5cf6;
  const heading = isAdded ? 'STAFF APPOINTMENT' : 'STAFF PROMOTION';
  const title = isAdded ? 'Welcome to the WCRP Staff Team' : 'Congratulations — You Have Been Promoted';
  const description = isAdded
    ? `You have officially joined the **WCRP Staff Team** in **${guild.name}**.`
    : `Your staff rank in **${guild.name}** has been updated. Your continued effort has been recognised by the team.`;

  const buildBaseEmbed = () => new EmbedBuilder()
    .setAuthor({ name: `WCRP Staff Utilities • ${heading}`, iconURL: guild.client.user.displayAvatarURL() })
    .setTitle(title)
    ...
    .addFields(
      ...
      { name: 'Status', value: isAdded ? 'Staff access granted' : 'Promotion approved', inline: true },
  ...
  if (channelId) {
    ...
    if (channel?.isTextBased()) {
      await channel.send({ embeds: [buildBaseEmbed()] }).catch(() => null);
    }
  }

  const dm = buildBaseEmbed()
    .setDescription(isAdded
      ? `Welcome to the **WCRP Staff Team**. Your staff appointment in **${guild.name}** has been confirmed.`
      : `Your promotion in **${guild.name}** has been confirmed. Thank you for your work and commitment.`)
    .addFields({
      name: 'Next Step',
      value: isAdded
        ? 'Please join the staff Discord below and review the staff information before taking your first actions.'
        : 'Please make sure you are using your new permissions and rank correctly across the staff team.',
      inline: false
    });
  ...
  await target.send({ embeds: [dm], components: [row] }).catch(() => null);
}
...
async function ensureRankRoleInGuild(guild, rank) {
  ...
  const sourceGuild = cfg?.sourceGuildId ? guild.client.guilds.cache.get(cfg.sourceGuildId) : null;
  const sourceRole = sourceGuild?.roles.cache.find(r => r.name.toLowerCase() === rank.toLowerCase());
  if (!guild.members.me?.permissions.has(PermissionFlagsBits.ManageRoles)) return null;
  role = await guild.roles.create({
    name: rank,
    color: sourceRole?.color ?? 0x5865f2,
    hoist: sourceRole?.hoist ?? false,
    mentionable: sourceRole?.mentionable ?? false,
    permissions: sourceRole?.permissions?.bitfield ?? 0n,
    reason: 'Create shared staff rank role'
  }).catch(() => null);
  return role;
}
...
function isEligibleTmod(userRecord) {
  return String(userRecord?.rank || '').toLowerCase() === TMOD_RANK.toLowerCase()
...
async function setStaffRankAcrossGuilds(client, userId, rank, reason='Staff rank update') {
  ...
  for (const guild of client.guilds.cache.values()) {
    ...
    const newRole = await ensureRankRoleInGuild(guild, rank);
...
function buildTmodEligibilityEmbed(guild, target, userRecord) {
  const strikes=activeStrikeCount(userRecord);
  const eligible=isEligibleTmod(userRecord);
  const hours=Number(userRecord?.hours || 0);
  const e=new EmbedBuilder()
    .setAuthor({ name:'PSRP Staff Utilities • PROMOTION SYSTEM', iconURL:guild.client.user.displayAvatarURL() })
    .setTitle(eligible ? 'Trial Moderator Promotion' : 'Trial Moderator Promotion Review')
    .setDescription(eligible
      ? `**${target.username}** is eligible to be promoted from **Trial Moderator** to **Moderator**.`
      : `**${target.username}** does not currently meet every Trial Moderator promotion requirement.`)
    .setThumbnail(target.displayAvatarURL({size:256}))
    .setColor(eligible ? 0xf59e0b : 0x64748b)
    .addFields(
      { name:'Candidate', value:`${target}\n	exttt{${target.id}}`, inline:true },
      { name:'Hours Completed', value:`**${hours.toFixed(2)}h** / **${TMOD_PROMOTION_HOURS.toFixed(2)}h**`, inline:true },
      { name:'Current Rank', value:`**${userRecord?.rank || 'Unranked'}**`, inline:true },
      { name:'Status', value:eligible ? '⏳ **PENDING INTERVIEW**' : `❌ Not eligible`, inline:false },
      { name:'Strike Check', value:strikes===0 ? '✅ No active strikes' : `❌ ${strikes} active strike${strikes===1?'':'s'}`, inline:true },
      { name:'Promotion Requirement', value:`Trial Moderator must have at least **${TMOD_PROMOTION_HOURS} hours** and **no active strike**.`, inline:true }
    )
...
async function handleTmodCompleteButton(interaction) {
  ...
  if (!isStaff(member)) {
    return interaction.reply({ embeds:[err('Access Denied','Staff permissions are required to complete this promotion interview.')], ephemeral:true });
  }
  const target = await guild.client.users.fetch(userId).catch(()=>null);
  if (!target) return interaction.reply({ embeds:[err('Promotion Failed','The candidate could not be found.')], ephemeral:true });
  const record=uid(userId,guild.id);
  if (!isEligibleTmod(record)) {
    return interaction.reply({ embeds:[err('No Longer Eligible',`This candidate no longer meets the **${TMOD_PROMOTION_HOURS} hour / no active strike** requirements.`)], ephemeral:true });
  }
  const oldRank=record.rank || TMOD_RANK;
  record.rank=MODERATOR_RANK;
  record.lastSeen=Date.now();
  saveStore();
  await interaction.deferUpdate();
  const results=await setStaffRankAcrossGuilds(guild.client,userId,MODERATOR_RANK,'Complete Trial Moderator promotion');
  await sendStaffPromotionNotification({guild,target,oldRank,newRank:MODERATOR_RANK,by:interaction.user.id,reason:'Completed Trial Moderator promotion interview',action:'promotion'});
  await sendLog(guild,`**Trial Moderator Promoted**\nUser: ${target}\nPrevious rank: ${oldRank}\nNew rank: ${MODERATOR_RANK}\nApproved by: <@${interaction.user.id}>\nReason: Completed Trial Moderator promotion interview`);
  const updated = buildTmodEligibilityEmbed(guild,target,record)
    .setTitle('Trial Moderator Promoted!')
    .setDescription(`**${target.username}** has successfully completed the Trial Moderator promotion process and is now **${MODERATOR_RANK}**.`)
    .setColor(0x22c55e)
    .spliceFields(3,1,{name:'Status',value:'✅ **PROMOTED TO MODERATOR**',inline:false})
    .addFields(
      { name:'Promoted By', value:`<@${interaction.user.id}>`, inline:true },
      { name:'Promotion Date', value:`<t:${Math.floor(Date.now()/1000)}:F>`, inline:true },
      { name:'Guild Updates', value: results.length ? results.map(r=>`${r.ok?'✅':'⚠️'} ${r.guildName}`).join('\n') : 'No shared guild members found.', inline:false }
    );
  const row=new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId(`tmod_completed:${userId}`).setLabel(`Completed By: ${interaction.user.username}`.slice(0,80)).setStyle(ButtonStyle.Success).setDisabled(true)
  );
...
**Assigned by:** ${interaction.user}`)]});
  ...
  if (name==='tmodpromos') {
    ...
    if (requested) {
      ...
      candidates.push([requested.id,r]);
    } else {
      for (const [id,r] of Object.entries(guildData.users)) {
        if (String(r.rank||'').toLowerCase()===TMOD_RANK.toLowerCase()) candidates.push([id,r]);
      }
    }
    const eligible=candidates.filter(([,r])=>isEligibleTmod(r));
    if (!eligible.length) return reply(interaction,{embeds:[embed('Trial Moderator Promotion Check',`No Trial Moderators currently meet the **${TMOD_PROMOTION_HOURS} hour** and **no active strike** requirements.`,0x64748b)]});
    const [firstId,firstRecord]=eligible[0];
    const firstUser=await interaction.client.users.fetch(firstId).catch(()=>null);
    if (!firstUser) return reply(interaction,{embeds:[err('Promotion Check Failed','The candidate could not be loaded.')],ephemeral:true});
    const firstEmbed=buildTmodEligibilityEmbed(interaction.guild,firstUser,firstRecord);
    const firstRow=new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`tmod_complete:${firstId}`).setLabel('Complete Interview').setStyle(ButtonStyle.Success));
    await interaction.reply({embeds:[firstEmbed],components:[firstRow]});
    for (const [id,r] of eligible.slice(1,10)) {
      const user=await interaction.client.users.fetch(id).catch(()=>null);
      if (!user) continue;
      const emb=buildTmodEligibilityEmbed(interaction.guild,user,r);
      const row=new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`tmod_complete:${id}`).setLabel('Complete Interview').setStyle(ButtonStyle.Success));
      await interaction.followUp({embeds:[emb],components:[row]});
    }
    return;
  }
  if (name==='promote-all-tmods') {
    ...
    const candidates=[];
    for (const [id,r] of Object.entries(guildData.users)) {
      if (isEligibleTmod(r)) candidates.push([id,r]);
    }
    ...
    for (const [id,r] of candidates) {
      ...
      if (!target) { failed.push(`<@${id}> — user unavailable`); continue; }
      const oldRank=r.rank || TMOD_RANK;
      r.rank=MODERATOR_RANK;
      r.lastSeen=Date.now();
      await setStaffRankAcrossGuilds(interaction.client,id,MODERATOR_RANK,'Bulk Trial Moderator promotion');
      await sendStaffPromotionNotification({guild:interaction.guild,target,oldRank,newRank:MODERATOR_RANK,by:interaction.user.id,reason:'Bulk Trial Moderator promotion',action:'promotion'});
      await sendLog(interaction.guild,`**Trial Moderator Promoted**\nUser: ${target}\nPrevious rank: ${oldRank}\nNew rank: ${MODERATOR_RANK}\nApproved by: <@${interaction.user.id}>\nReason: Bulk Trial Moderator promotion`);
      promoted.push(`<@${id}> — ${r.hours.toFixed(2)}h`);
    }
    saveStore();
    const out=embed('Trial Moderators Promoted',`Automatically promoted **${promoted.length}** eligible Trial Moderator${promoted.length===1?'':'s'} to **${MODERATOR_RANK}**.`,0x22c55e)
      .addFields({name:'Promoted',value:promoted.join('\n').slice(0,1024) || 'None',inline:false});
    if (failed.length) out.addFields({name:'Could Not Process',value:failed.join('\n').slice(0,1024),inline:false});
